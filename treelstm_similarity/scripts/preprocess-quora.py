...
def dependency_parse(filepath, cp='', tokenize=True):
    ...
    os.system(cmd)
def divide_proc(data_dir):
    a = ["dev","train","test"]
    for folder in a:
        first_sent_folder = os.path.join(data_dir, folder,"1")
        second_sent_folder = os.path.join(data_dir, folder,"2")
        make_dirs([first_sent_folder,second_sent_folder])
        with open(os.path.join(data_dir,folder,"sents.tsv")) as sents, \
            open(os.path.join(first_sent_folder,"sents_proc.txt"), 'w') as first_sent_proc, \
            open(os.path.join(data_dir, folder, "labels.txt"), 'w') as labels, \
            open(os.path.join(second_sent_folder,"sents_proc.txt"), 'w') as second_sent_proc:

            for sent in sents:
                splits = sent.split("\t")
                labels.write(splits[0])
                labels.write("\n")
                first_sent_proc.write(splits[1])
                first_sent_proc.write("\n")
                second_sent_proc.write(splits[2])
                second_sent_proc.write("\n")

if __name__ == '__main__':
    print('=' * 80)
    print('Preprocessing quora dataset')
    print('=' * 80)

    base_dir = os.path.dirname(os.path.dirname(os.path.realpath(__file__)))
    data_dir = os.path.join(base_dir, 'quora_data')
    ...
    divide_proc(data_dir)
    sent_paths = glob.glob(os.path.join(data_dir, '*/*/sents_proc.txt'))
    print(sent_paths)
    ...
    # get vocabulary
    build_vocab(sent_paths, os.path.join(data_dir, 'vocab.txt'))
    build_vocab(sent_paths, os.path.join(data_dir, 'vocab-cased.txt'), lowercase=False)
